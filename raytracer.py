...
class Raytracer(object):
  ...
  def render(self, effect3d=False):
    fov = int(pi/2)
    for y in range(self.height):
      for x in range(self.width):
        ...
        if(effect3d):
          eye1 = self.cast_ray(V3(0.3,0,0), direction)
          eye2 = self.cast_ray(V3(-0.3,0,0), direction)
          if not eye1.equals(self.background_color):
            eye1 = eye1*0.6 + color(100,0,0)                                    
          if not eye2.equals(self.background_color):
            eye2 = eye2*0.6 + color(0,0,100)                                            
          eye_sum = eye1 + eye2
